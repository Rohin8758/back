      ...
      if (addToCart) {
        ...
        for (let i = 0; i < checklengthofproduct; i++) {
          ...
          if (addToCart.user.product[i].product_id == productId) {
            addToCart.user.product[i].quantity =
              addToCart.user.product[i].quantity + 1;
...
router.get("/getalldataofcart/:id", async (req, res) => {
  try {
    ...
    console.log(userId);
    if (userId) {
      const addToCartData = await AddToCart.find({ "user.user_id": userId });
      if (!addToCartData) {
        return res.status(404).json({
          status: 404,
    ...
    addToCartData[0].user.product.forEach(async (element) => {
      // console.log(element);
      // console.log("..");
      ...
      cartData = {
        cartId: addToCartData[0]._id,
        user: {
          product: productArray,
          userId: addToCartData[0].user.user_id,
        },
      };
    });
        ...
        for (let i = 0; i < cartData[0].user.product.length; i++) {
          if (cartData[0].user.product[i].product_id == productId) {
            ...
            if (cartData[0].user.product[i].quantity > 1) {
              cartData[0].user.product[i].quantity =
                cartData[0].user.product[i].quantity - 1;
            ...
            try {
              if (cartData[0].user.product[i].quantity == 1) {
                const deleteData = cartData[0].user.product.remove(
                  cartData[0].user.product[i]
                );
                await cartData[0].save({ validateBeforeSave: false });
                return res.status(200).json({
                  status: 200,
