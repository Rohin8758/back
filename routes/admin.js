...
router.post("/changepassword/:id", async (req, res) => {
  try {
    ...
    try {
      const checkPasswordIsSameOrNot = await bcrypt.compare(
        req.body.password,
        admin.password
      );
...
// -------------------- Add Product --------------------
router.post("/addproduct",uploadStorage.array("images", 10),
  async (req, res, next) => {
    try {
...
// -------------------- Update Only Image ------------------
router.patch("/updateProductimages/:id",uploadStorage.array("images", 10),
  async (req, res, next) => {
    const id = req.params.id;
    const product = await Product.findById(id);
    try {
    ...
    try {
      if (req.files.length > 0) {
        //this condition is for check Admin send file or not if Admin not send file cloudinary image is not doing delete.
...
router.patch("/updateproductdata/:id", async (req, res, next) => {
  ...
  try {
    ...
    try {
      if (req.body.title != "") {
        const checkTitleIsValidOrNot = await Product.findOne({
          title: req.body.title,
        });
    ...
    try {
      ...
      res.status(500).json({
        staus: 500,
        message: "Cann not Update description",
        error: error.message,
      });
    }
    ...
    try {
      if (req.body.producttype == "") {
        return res.status(400).json({
          staus: 400,
          message:
            "Enter Valid Product Type like [hot item, new arrival, on sale] This.",
        });
      }
